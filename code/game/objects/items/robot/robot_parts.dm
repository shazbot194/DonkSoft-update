...
/obj/item/robot_parts/eyebot/New()
	..()
	src.updateicon()
...
//			large					//
//////////////////////////////////////
/obj/item/robot_parts/large
	name = "large_frame"
	desc = "An large frame."
	icon_state = "large_frame"
	...
	var/obj/item/weapon/stock_parts/cell/cell = null
	var/obj/item/robot_parts/lhead/head = null
	var/obj/item/stack/cable_coil/cable = null
	var/obj/item/stack/sheet/plasteel/metal = null

/obj/item/robot_parts/large/New()
	..()
	src.updateicon()

/obj/item/robot_parts/large/proc/updateicon()
	src.overlays.Cut()
	if(head)
		src.overlays += "large_head"

/obj/item/robot_parts/large/proc/check_completion()
	if(src.cable && src.cell)
		if(src.head && src.metal)
			feedback_inc("cyborg_frames_built",1)
			return 1
	return 0

/obj/item/robot_parts/large/attackby(obj/item/W as obj, mob/user as mob, params)
	..()
	if(istype(W, /obj/item/robot_parts/lhead))
		if(src.head)	return
		if(W:flash2 && W:flash1)
			user.drop_item()
			W.loc = src
	...
	if(istype(W, /obj/item/stack/sheet/plasteel))
		var/obj/item/stack/sheet/plasteel/metal = W
		if(src.metal)
			to_chat(user, "\blue The bot already has plating installed.")
			return 0
		else if(metal.amount < 6)
			to_chat(user, ("There's not enough plasteel."))
			return 0
		else
			metal.use(6)
...
/obj/item/robot_parts/lhead
	name = "Large head"
	desc = "A large reinforced braincase, with spine-plugged neural socket and sensor gimbals."
	icon_state = "large_head"
	part = list("lhead")
	var/obj/item/device/flash/flash1 = null
	var/obj/item/device/flash/flash2 = null

/obj/item/robot_parts/lhead/attackby(obj/item/W as obj, mob/user as mob, params)
	..()
	if(istype(W, /obj/item/device/flash))
		if(istype(user,/mob/living/silicon/robot))
			to_chat(user, "\red How do you propose to do that?")
